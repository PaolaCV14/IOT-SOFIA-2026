...
\begin{itemize}
\item \textbf{Perception and High-Level Control:} A workstation executes an optimized Python tracking pipeline utilizing MediaPipe and OpenCV. It captures real-time video, tracks 3D upper-body pose landmarks and dual 21-point hand skeletons, classifies interaction gestures (dynamic waving, heart formations, rock-paper-scissors intent), and maps Cartesian targets $\mathbf{p}(t) \in \mathbb{R}^3$ into spatial trajectory coordinates.
\item \textbf{Master Communication and Telemetry:} An ESP32-C3 master controller receives spatial setpoints via low-latency Wi-Fi / BLE communication and broadcasts synchronized module packets across an isolated RS-485 / Modbus serial bus.
...
conducted in a virtual 3D environment using the Unity game engine. The objective was to evaluate
