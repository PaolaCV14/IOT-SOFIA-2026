...
\begin{equation}
e(t) = \theta_{\text{ref}}(t) - \theta(t),
\label{eq:position_error}
\end{equation}
where $\theta_{\text{ref}}(t) \in [\theta_{\min}, \theta_{\max}]$ denotes the commanded angular setpoint, and $\theta(t)$ denotes the instantaneous angular position of the output shaft.

Under ideal, unconstrained operation, the internal feedback loop drives $e(t) \to 0$ within a settling time $t_s \in [75, 180]\text{ ms}$. However, when an external mechanical barrier or physical stall enforces a boundary condition such that $\theta(t) = \theta_{\text{stall}} \neq \theta_{\text{ref}}(t)$, the error persists indefinitely:
\begin{equation}
e(t) = \theta_{\text{ref}}(t) - \theta_{\text{stall}} = \Delta\theta_0 \neq 0, \quad \forall t \ge t_0.
...
Assuming the servomotor housing behaves as a lumped thermal capacitance with convective and conductive dissipation to the ambient air, the thermal accumulation dynamics under constant electrical power dissipation can be modeled by a first-order differential equation:
\begin{equation}
C_{\text{th}} \frac{dT(t)}{dt} + \frac{T(t) - T_{\text{amb}}}{R_{\text{th}}} = P_{\text{diss}},
\label{eq:thermal_diff}
\end{equation}
where $C_{\text{th}}$ represents the lumped thermal capacitance, $R_{\text{th}}$ represents the equivalent thermal resistance, and $P_{\text{diss}} = I_{\text{stall}}^2 R_{\text{coil}}$ is the continuous stall power.

Integrating (\ref{eq:thermal_diff}) with the initial condition $T(0) = T_{\text{amb}}$ yields the standard step response:
\begin{equation}
T(t) = T_{\text{amb}} + \Delta T_{\max} \left(1 - e^{-t/\tau}\right),
\label{eq:first_order_thermal}
\end{equation}
where $\Delta T_{\max} = T_{\text{max}} - T_{\text{amb}} = P_{\text{diss}} R_{\text{th}}$ denotes the maximum asymptotic temperature rise, and $\tau = R_{\text{th}} C_{\text{th}}$ is the thermal time constant of the actuator.

To identify $\tau$ from the empirical data recorded in Trial 1 without non-linear curve fitting, (\ref{eq:first_order_thermal}) is algebraically rearranged into a normalized logarithmic linear form:
\begin{equation}
y(t) = \ln\left(1 - \frac{T(t) - T_{\text{amb}}}{\Delta T_{\max}}\right) = -\frac{1}{\tau} t.
\label{eq:linearized_thermal}
\end{equation}

Applying linear least-squares regression over the active heating interval $t \in [1, 226]\text{ s}$ yields an empirical slope of $m = -0.01241\text{ s}^{-1}$. Consequently, the identified thermal time constant is:
\begin{equation}
\tau = -\frac{1}{m} \approx 80.6\text{ s}.
\label{eq:identified_tau}
\end{equation}

This identification indicates that when an MG90S servomotor enters an unmitigated mechanical stall, it reaches $63.2\%$ of its maximum temperature rise ($\approx 48.9^\circ\text{C}$) in approximately $80\text{ s}$, and approaches the destructive threshold of $65^\circ\text{C}$ in under 4 minutes. Because the internal driver continuously applies peak torque without timeout or thermal throttling, its behavior closely mirrors an unconstrained bang-bang controller under saturation.
...
\subsection{Preliminary Hardware and Custom PCB Design}
\label{sec:pcb_design}

In the current prototype stage (D-Lab corte 1), the system operates using breadboarded breakouts connecting the ESP32-C3 development board and dual PCA9685 driver modules. To satisfy the footprint, cabling density, and current capacity requirements of the 16-module kinetic honeycomb, preliminary PCB schematics and design criteria have been established:
\begin{enumerate}
\item \textbf{High-Current Power Distribution:} 32 simultaneous MG90S servomotors exhibit a collective peak stall current demand of up to $16\text{--}24\text{ A}$ at $5.0\text{ V}$. The preliminary custom carrier board incorporates wide copper power planes, dedicated terminal blocks, and bulk electrolytic decoupling capacitors ($4700\,\mu\text{F}$) to suppress inductive voltage transients and brownouts.
\item \textbf{Industrial Bus Integration:} Onboard transceiver circuitry (MAX485 / SP3485) interfaces the ESP32-C3 UART pins with termination resistors for noise-immune RS-485 balanced transmission across the multi-drop module network.
\item \textbf{Level Shifting and Logic Isolation:} Bidirectional I$^2$C level shifters protect the 3.3~V ESP32-C3 GPIOs when driving 5.0~V PCA9685 control rails.
\end{enumerate}
...
%
%\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{fig1}}]{First Last}
